...
def generate_dataset(model, train_loader, valid_dataset, device,batch_size,resultpath, alpha = 0.0, start_point = "gaussian", 
    ...
    test_model_list = []
    for i in range(10):
        test_model_list.append(resnet.ResNet18().to(device))
    ...
    for idx, (data, target) in enumerate(train_loader):
        ...
        # test_conv_out_layers = []
        # if conv_out_bool == True:
        #     for module in test_model.modules():
        #         if isinstance(module, nn.Conv2d):
        #             test_conv_out_layers.append(ConvOutHook(module))
        ...
        for inner in range(max_iterations):
            ...
            for test_model in test_model_list:
                rand_feature_diff += loss_feature_diff(test_model, data, data_r, target, None, loss_type)
            rand_feature_diff /= len(test_model_list)
            ...
            # loss_fn = feature_diff - weight * image_diff + weight_conv*conv_diff
            ...
            if inner % inner_per_distance_record == 0:
                ...
                test_feature_diff_history.append(rand_feature_diff.item())
...
if __name__ == "__main__":
    ...
    resultpath = 'rand_loss_visualweight'+str(weight).replace('.', '_')+'cov'+str(weight_conv).replace('.', '_')+\
