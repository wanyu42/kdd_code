...
def init_gen(data, valid_dataset, target, device, start_point, noise_level, alpha, swap_num):
    ...
    elif start_point == "half":
        data_r = []
        #### Upper part is from held-out 50*10
        if target.shape[0]>swap_num: 
            r_idx = torch.randint(0, len(valid_dataset), (target.shape[0]-swap_num, ))
            for i in r_idx:
                data_r.append(valid_dataset[i][0])              
            data_r = torch.stack(data_r).to(device)
            #### Bottom part is from permutation
            data_r_last = data[torch.randperm(swap_num)+target.shape[0]-swap_num]
        else:
            data_r = torch.tensor(data_r).to(device).view(-1, 3, 32, 32)
            data_r_last = data[torch.randperm(target.shape[0])]
        
        data_r = torch.vstack([data_r, data_r_last]).to(device).requires_grad_(True)
...
def conv_loss(conv_out, conv_out_r, batch_size, inner):
    loss = 0
    for conv_layer_out, conv_layer_out_r in zip(conv_out, conv_out_r):
        loss += ((conv_layer_out - conv_layer_out_r)**2).sum() / batch_size
    # import ipdb; ipdb.set_trace()
    if inner % 100 == 0:
        print(f'conv loss {1 * loss.item():4.2f}')
    return 1 * loss


def loss_feature_diff(model, data, data_r, target, inner, conv_out_layers, loss_type='l2', conv=False):
    with torch.no_grad():
    ...
    if conv:   
        feature_diff += conv_loss(conv_out[-10:-5], conv_out_r[-10:-5], target.shape[0], inner)
...
def gd_step(loss_fn, data_r, target):
    grad = torch.autograd.grad(loss_fn, data_r,
                                    retain_graph=False, create_graph=False)[0]

    data_r = data_r.detach() - 0.01 * grad
    data_r = torch.clamp(data_r, min=0.0, max=1.0).detach()
...
def generate_dataset(model, train_loader, valid_dataset, device,batch_size,resultpath, alpha = 0.0, start_point = "gaussian", 
        loss_type = "l2",weight = 1e-2, noise_level = 0.2, swap_num = 128, conv=False,
    ...
    for idx, (data, target) in enumerate(train_loader):
        ...
        data_r = init_gen(data, valid_dataset, target, device, start_point, noise_level, alpha, swap_num)        
        ...
        for inner in range(max_iterations):
            ...
            feature_diff = loss_feature_diff(model, data, data_r, target, inner, conv_out_layers, loss_type, conv)
            ...
            # data_r = gd_step(loss_fn, data_r, target)
...
if __name__=="__main__":
    ...
    generate_dataset(model, train_loader, valid_dataset, device,batch_size,resultpath, alpha = alpha, start_point = start_point, 
        loss_type = feature_loss, weight = weight, noise_level = noise_level, conv=conv,
